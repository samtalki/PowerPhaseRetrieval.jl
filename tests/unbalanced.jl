...
vm,va = zeros(Float64,3*K),zeros(Float64,3*K) #voltage magnitudes and angles (3)
p,q = zeros(Float64,3*K),zeros(Float64,3*K) #active and reactive power net injections

#--- voltage magnitude and phase angles
for (bus_id,bus_sol) in result["solution"]["bus"]
    i = math["bus"][bus_id]["index"]
    vm[3*i-2:3*i] .= bus_sol["vm"]
    va[3*i-2:3*i] .= bus_sol["va"]
end
...
vph = vm .* cis.(va) # voltage phasors
s = p + im.*q # complex power injections
...
#----- test the symmetry hypothesis of the Jacobian matrices
err_dpθ = norm(dpθ(vm,va) - dpξ(vm,va,q))/norm(dpθ(vm,va))
err_dqθ = norm(dqθ(vm,va) - dqξ(vm,va,p))/norm(dqθ(vm,va))