...
# ╔═╡ 73fd7462-65ea-11ed-226d-995a20ee4657
using LinearAlgebra,Plots,JuMP,SCS

# ╔═╡ 18b7f180-646b-4f97-a1f8-25f0d996dc88
begin
	#Admittance
	Y = [-20 10 10;
		10 -20 10;
...
# ╔═╡ ce0db0c8-1452-4fca-a680-ed7c3d8fb778
"""
Mismatch function
"""
f(x)= [
	10*x[3]*sin(x[1])+10*x[3]*sin(x[1]-x[2]) + 3,
	10*sin(x[2]) + 10*x[3]*sin(x[2]-x[1])-0.8,
	-10*x[3]*cos(x[1]) + 20*x[3]^2 - 10*x[3]*cos(x[1]-x[2]) + 1.2
	]


# ╔═╡ 02b8d7db-1b12-43df-bd61-37e38d503ff2
begin #Injection expressions
	p2(x) = 10*x[3]*sin(x[1])+10*x[3]*sin(x[1]-x[2])
	p3(x) = 10*sin(x[2]) + 10*x[3]*sin(x[2]-x[1])
	q2(x) = -10*x[3]*cos(x[1]) + 20*x[3]^2 - 10*x[3]*cos(x[1]-x[2])
end

# ╔═╡ c3ad7a9c-f756-4e27-8a7a-7fd3bb0f4fb6
"""
Jacobian function
"""
function J(x)
	J = zeros(3,3)
	J[1,1] = 10*x[3]*cos(x[1]) + 10*x[3]*cos(x[1] - x[2]) #dp2/dth2
	J[1,2] = -10*x[3]*cos(x[1] - x[2])  #dp2/dth3
	J[1,3] = 10*sin(x[1]) + 10*sin(x[1] - x[2]) #dp2/dv2
	J[2,1] = -10*x[3]*cos(x[2] - x[1]) #dp3/dth2
	J[2,2] = 10*cos(x[2]) + 10*x[3]*cos(x[2] - x[1]) #dp3/dth3
	J[2,3] =  10*sin(x[2] - x[1]) #dp3/dv2
	J[3,1] = 10*x[3]*sin(x[1]) + 10*x[3]*sin(x[1] - x[2]) #dq2/dth2
	J[3,2] = -10*x[3]*sin(x[1] - x[2]) #dq2/dth3
	J[3,3] =  -10*cos(x[1])+40*x[3]-10*cos(x[1]-x[2]) #dq2/dv2
	return J
end

# ╔═╡ c4a48c3b-c279-48bb-9590-fff840ec5625
"""
Phaseless jacobian function
"""
function J_phaseless(x)
	J_full = J(x)
	
	
end

# ╔═╡ 90b5ff8c-2875-4c1d-aac9-1e2f3710c546
"""
Given guess for initial state and tolerance ϵ solve the Newton Raphson Power Flow
"""
function solve_nr_pf(x_0,ϵ=0.0001)
...
# ╔═╡ ca44b36b-196b-4a85-b2a9-d83a3008a366
"""
Get the phaseless PQ jacobian for the phase retrieval problem.
J(x) = dp/dv + j dp/dv.
"""
function Jf(x_k)
	J_k = J(x_k)
	#Jf_k = dp2/dv2 + jdq2/dv2
	Jf_k = J[1,3] + J[3,3]*im
	return Jf_k
end

# ╔═╡ 33a6cb75-a9e3-41ee-9163-ddd0229a66cd
function solve_phaseless_nr(xhat_0=1)
end

# ╔═╡ 5d976a75-f3c3-4a4d-880b-5d533f4a7996
"""
...
# ╔═╡ a6b934b8-f957-483e-b849-342d22d9cc15
f0 = f(x0) #Initial mismatches

# ╔═╡ bea3acf0-3c79-4e77-9298-8add0085c74f
J0= J(x0) #Initial jacobian

# ╔═╡ 321d3e33-b846-47bc-aa04-fffde579cbb7
f_iter,x_iter,J_iter = solve_nr_pf(x0,)
